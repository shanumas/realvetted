...
import { Mail, MessageSquare, Users, FileText, AlertCircle } from "lucide-react";
...
export default function EmailOutbox() {
  ...
  const sentEmails = (activeTab === "all" ? allEmails : userEmails)?.filter(
    (email) => email.status === "sent"
  ) || [];
  
  const failedEmails = (activeTab === "all" ? allEmails : userEmails)?.filter(
    (email) => email.status === "failed"
  ) || [];

  const isLoading = activeTab === "all" ? isLoadingAllEmails : isLoadingUserEmails;
  const emails = activeTab === "all" ? allEmails : userEmails;
