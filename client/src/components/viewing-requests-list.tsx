...
export function ViewingRequestsList({
  userId,
  role,
}: ViewingRequestsListProps) {
  ...
  const [lastUpdateMessage, setLastUpdateMessage] = useState<string | null>(
    null,
  );
  const [showUpdateBanner, setShowUpdateBanner] = useState(false);
  const [showAgreementModal, setShowAgreementModal] = useState(false);
  const [selectedRequest, setSelectedRequest] =
    useState<ViewingRequestWithParticipants | null>(null);
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            console.log(
              "Invalidating viewing requests queries due to status update:",
              data.data.status,
            );
            ...
            if (data.data.propertyId) {
              queryClient.invalidateQueries({
                queryKey: [
                  `/api/properties/${data.data.propertyId}/viewing-requests`,
                ],
              });
            }
          } else {
  ...
  const filteredRequests = viewingRequests?.filter((request) => {
    ...
    if (activeTab === "rejected")
      return request.status === "rejected" || request.status === "cancelled"; // Handle both spellings
  ...
  const updateRequestMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          "The viewing request status has been updated successfully.",
      });
    ...
    onSuccess: (data) => {
      ...
      if (viewingRequests && viewingRequests.length > 0) {
        ...
        propertyIds.forEach((propId) => {
          queryClient.invalidateQueries({
            queryKey: [`/api/properties/${propId}/viewing-requests`],
          });
        });
      }
    },
  ...
  const sellerAgentApprovalMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/viewing-requests/${id}/seller-agent-approval`,
        { approvalStatus },
      );
      return response.json();
    },
  ...
  const buyerAgentApprovalMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/viewing-requests/${id}/buyer-agent-approval`,
        { approvalStatus },
      );
      return response.json();
    },
    ...
    if (confirm("Are you sure you want to cancel this viewing request?")) {
      try {
        ...
        toast({
          ...
          description:
            "There was a problem cancelling your viewing request. Please try again.",
          variant: "destructive",
        });
      }
    }
  ...
  const pendingCount =
    viewingRequests?.filter((req) => req.status === "pending").length || 0;
  const approvedCount =
    viewingRequests?.filter((req) => req.status === "accepted").length || 0; // Use 'accepted' status from backend
  const rejectedCount =
    viewingRequests?.filter(
      (req) => req.status === "rejected" || req.status === "cancelled",
    ).length || 0;
  const completedCount =
    viewingRequests?.filter((req) => req.status === "completed").length || 0;
                          ...
                          <div className="flex items-center">
                            ...
                            <span className="text-sm">
                              {formatCaliforniaTime(
                                request.requestedDate.toString(),
                                "MMMM d, yyyy",
                              )}
                            </span>
                          </div>
                          ...
                          {role === "buyer" &&
                            (request.status === "pending" ||
                              ...
                              >
                                ...
                                request.id ===
                                  (cancelRequestMutation.variables as number) ? (
                                ...
                                request.buyerAgentApprovalStatus ===
                                  ...
                                  <>
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        buyerAgentApprovalMutation.mutate({
                                          id: request.id,
