 ...
 */
export async function extractPropertyFromRealtorUrl(
  url: string,
  ...
  try {
    ...
    console.log("Fetched Realtor.com HTML content:", htmlContent);
...
function parseRealtorHtml(html: string, url: string): PropertyAIData {
  ...
  if (
    html.includes("Your request could not be processed") ||
    html.includes("detected as a bot") ||
    html.includes("CAPTCHA")
  ) {
    throw new Error(
      "Realtor.com has blocked this request. Try again later or from a different network.",
    );
  }
  ...
  try {
    ...
    if (bedsBathsArea) {
      const bedsElement = bedsBathsArea.querySelector(
      ...
      const bathsElement = bedsBathsArea.querySelector(
      ...
      const sqftElement = bedsBathsArea.querySelector(
        '[data-testid="property-meta-sq-ft"]',
      );
      if (sqftElement) {
    ...
    const agentSection = doc.querySelector(
      '[data-testid="listing-agent-container"]',
    );
    if (agentSection) {
      const agentName = agentSection.querySelector(
      ...
      const agentCompany = agentSection.querySelector(
        '[data-testid="listing-agent-brokerage"]',
      );
      if (agentCompany) {
        propertyData.listingAgentCompany =
          agentCompany.textContent?.trim() || "";
      }

      const agentPhone = agentSection.querySelector(
        '[data-testid="listing-agent-phone"]',
      );
      if (agentPhone) {
        propertyData.listingAgentPhone =
          agentPhone.textContent?.trim().replace(/[^0-9]/g, "") || "";
      }
    }

    // Extract property details
    const propertyDetails = doc.querySelector(
      '[data-testid="property-detail-container"]',
    );
    if (propertyDetails) {
      const yearBuiltSection = Array.from(
        propertyDetails.querySelectorAll("div"),
      ).find((div) => div.textContent?.includes("Year built"));

      if (yearBuiltSection) {
        const yearMatch =
          yearBuiltSection.textContent?.match(/Year built[:\s]+(\d{4})/) || [];
        if (yearMatch[1]) {
          propertyData.yearBuilt = Number(yearMatch[1]);
        }
      }

      const propertyTypeSection = Array.from(
        propertyDetails.querySelectorAll("div"),
      ).find((div) => div.textContent?.includes("Property type"));

      if (propertyTypeSection) {
        const typeMatch =
          propertyTypeSection.textContent?.match(
            /Property type[:\s]+(.+?)(?:$|\n)/,
          ) || [];
        if (typeMatch[1]) {
 ...
 */
export async function findRealtorUrlFromOtherSite(
  url: string,
): Promise<string | null> {
  try {
  ...
  try {
    ...
    if (domain.includes("zillow.com")) {
      ...
      if (addressMatch && addressMatch[1]) {
        ...
        const cityStateMatch = addressPart.match(
          /([A-Za-z\s]+)\s([A-Z]{2})\s(\d{5})/,
        );
        if (cityStateMatch) {
          propertyData.address = addressPart
            .replace(cityStateMatch[0], "")
            .trim();
    ...
    if (
      ...
      propertyData.address !== "Address unavailable"
    ) {
 ...
 * @returns The extracted property data
 */
export async function extractPropertyFromAnyUrl(
  url: string,
): Promise<PropertyAIData> {
  try {
    ...
    if (realtorUrl) {
      ...
      try {
        ...
        console.error(
          "Failed to extract from Realtor.com equivalent:",
          realtorError,
        );
      ...
      console.log(
        "No Realtor.com equivalent found. Using URL analysis fallback.",
      );
    }
    ...
    return {
      ...
      _extractionMethod: "url-analysis",
      _extractionSource:
        "Limited data extracted from URL structure. No Realtor.com equivalent found or extraction failed.",
    };
  } catch (error) {
