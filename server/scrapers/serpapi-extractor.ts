...
function extractAddressFromUrl(
  url: string,
): { streetAddress: string; city: string; state: string; zip: string } | null {
  try {
    // Create URL object
    ...
    if (url.includes("zillow.com")) {
      ...
      if (addressMatch && addressMatch[1]) {
        ...
        for (let i = 0; i < addressParts.length; i++) {
          if (
            addressParts[i].length === 2 &&
            addressParts[i] === addressParts[i].toUpperCase()
          ) {
            stateIndex = i;
            break;
          }
    ...
    try {
      const addressLikeRegex =
        /(\d+)[-\s]+([^-\s]+)[-\s]+([^-\s]+)[-\s]+([^-\s,]+)[,-\s]+([A-Za-z]{2})(?:[-\s]+(\d{5}))?/;
