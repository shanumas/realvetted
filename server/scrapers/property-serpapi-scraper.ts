  ...
  if (isUrl) {
    ...
    try {
      ...
      if (domain.includes("zillow.com")) {
        ...
        const addressPart = pathParts.find(
          (part) =>
            part.includes("-") &&
            (part.includes("CA-") ||
              part.includes("NY-") ||
        ...
        // For Redfin URLs
        const addressParts = pathParts.filter(
          (part) => !part.includes("home") && !part.match(/^\d+$/),
        );
    ...
    searchQuery = url + " listed by details";
  }
  ...
  try {
    ...
    if (data.organic_results && data.organic_results.length > 0) {
      ...
      const relevantResults =
        realEstateResults.length > 0 ? realEstateResults : data.organic_results;

      if (relevantResults.length > 0) {
        const mainResult = relevantResults[0];

        console.log(JSON.stringify(relevantResults, null, 2)); // pretty‑print
        ...
        if (mainResult.snippet) {
          ...
          const bathsMatch = mainResult.snippet.match(
            /(\d+(?:\.\d+)?)\s*bath/i,
          );
          if (bathsMatch) {
          ...
          const sqftMatch = mainResult.snippet.match(
            /(\d+(?:,\d+)?)\s*sq\s*ft/i,
          );
          if (sqftMatch) {
        ...
        if (
          mainResult.link &&
          (mainResult.link.includes("zillow.com") ||
      ...
      if (data.knowledge_graph) {
        ...
        if (kg.listing_agent || kg.agent || kg.real_estate_agent) {
          const agentInfo =
            kg.listing_agent || kg.agent || kg.real_estate_agent;
          ...
          if (typeof agentInfo === "string") {
            ...
            if (license) {
              propertyData.listingAgentLicenseNo =
                cleanLicenseNumber(license) || license;
            }

            // Company or brokerage
            propertyData.listingAgentCompany =
              agentInfo.company || agentInfo.brokerage || "";
          }
        }
    ...
    throw new Error(
      `Failed to extract property data using SerpAPI: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
