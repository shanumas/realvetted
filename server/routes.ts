...
`;
  ...
  // Endpoint for seller to sign agency disclosure form
  app.post("/api/properties/:id/sign-agency-disclosure", isAuthenticated, async (req, res) => {
    try {
      const propertyId = parseInt(req.params.id);
      const { agreementId, sellerSignature } = req.body;
      ...
      // Verify that the current user is the seller of this property
      if (property.sellerId !== (req.user as User).id || (req.user as User).role !== 'seller') {
        return res.status(403).json({
          success: false,
          error: "Only the seller of this property can sign this document"
        });
      }
      ...
      // Check that the agreement belongs to this property
      if (agreement.propertyId !== propertyId) {
        return res.status(400).json({
          success: false,
          error: "Agreement does not belong to this property"
        });
      }
      
      // Check that the agreement is in pending_seller state
      if (agreement.status !== "pending_seller") {
        return res.status(400).json({
          success: false,
          error: "This agreement is not pending seller signature"
        });
      }
      
      // Get buyer and agent information if available
      const buyer = property.buyerId ? await storage.getUser(property.buyerId) : undefined;
      const agent = property.agentId ? await storage.getUser(property.agentId) : undefined;
      
      // Check if a previous version of this form already exists
      // If so, we'll want to use the same form data for consistency
      let pdfBuffer: Buffer;
      let pdfUrl: string;
      
      try {
        // Basic agency disclosure form data
        const formDataForPdf: AgencyDisclosureFormData = {
          // Buyer info
          buyerName1: buyer ? `${buyer.firstName} ${buyer.lastName}` : "",
          
          // Seller info
          sellerName1: property.seller ? `${property.seller.firstName} ${property.seller.lastName}` : "",
          ...
          agentBrokerageName: "Real Estate Professionals, Inc.",
          agentLicenseNumber: agent ? "CA123456" : "", // This should be replaced with actual license number
        };
        
        // Fill the PDF form with data
        pdfBuffer = await fillAgencyDisclosureForm(formDataForPdf);
        
        // Add the buyer signature if it exists
        if (agreement.buyerSignature) {
          pdfBuffer = await addSignatureToPdf(pdfBuffer, agreement.buyerSignature, 'buyer1');
        }
        
        // Add the agent signature if it exists
        if (agreement.agentSignature) {
          pdfBuffer = await addSignatureToPdf(pdfBuffer, agreement.agentSignature, 'agent');
        }
        
        // Add seller signature to the PDF
        if (sellerSignature) {
          pdfBuffer = await addSignatureToPdf(pdfBuffer, sellerSignature, 'seller1');
        }
        
        // Generate a unique filename
        const timestamp = Date.now();
        const filename = `agency_disclosure_${propertyId}_${timestamp}.pdf`;
        const newPdfPath = path.join(process.cwd(), 'uploads', 'agreements', filename);
        ...
        // Update the agreement with the seller signature
        await storage.updateAgreement(agreementId, {
          sellerSignature,
          status: "completed", // Update status to completed since both parties have signed
          signedAt: new Date(),
          url: pdfUrl
        });
        
        // Add a property activity log entry
        await storage.createPropertyActivityLog({
          propertyId,
          userId: (req.user as User).id,
          activity: "Seller signed agency disclosure form",
          details: {
            agreementId,
            agreementType: "agency_disclosure"
          }
        ...
        // If we have a WebSocket server, notify the agent
        if (property.agentId && websocketServer) {
          websocketServer.broadcastToUsers([property.agentId], {
            type: "notification",
            data: {
              message: `The seller has signed the agency disclosure form for property at ${property.address}.`,
              propertyId
            }
          });
        }
        ...
        return res.json({
          success: true,
          message: "Agency disclosure form signed successfully",
          agreementUrl: pdfUrl
        });
